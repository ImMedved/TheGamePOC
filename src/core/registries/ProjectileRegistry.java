...
public final class ProjectileRegistry {
    ...
    public ProjectileDefinition get(int id) {
        if (id < 0 || id >= definitions.length) throw new IllegalArgumentException("Invalid projectile id: " + id);

        ProjectileDefinition def = definitions[id];
        if (def == null) throw new IllegalStateException("Projectile not registered: " + id);
    ...
    private void ensureCapacity(int id) {
        if (id >= definitions.length) {
            ProjectileDefinition[] newArr = new ProjectileDefinition[id + 1];
